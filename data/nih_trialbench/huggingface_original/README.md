...
You can load each config with the 🤗 `datasets` library:
...
patients = load_dataset("ncbi/NIH-TrialBench", "patients", split="test")
annotations = load_dataset("ncbi/NIH-TrialBench", "clinician_annotations", split="test")
trials = load_dataset("ncbi/NIH-TrialBench", "trial_search_space", split="test")
...
To export the three configs back to JSONL files:

```python
from datasets import load_dataset
from pathlib import Path

out_dir = Path("ncbi/NIH-TrialBench")
out_dir.mkdir(exist_ok=True)

for config in ["patients", "clinician_annotations", "trial_search_space"]:
    ds = load_dataset("ncbi/NIH-TrialBench", config, split="test")
    ds.to_json(out_dir / f"{config}.jsonl", orient="records", lines=True)
```
...
patients = load_dataset(
    "json",
    data_files="hf://datasets/ncbi/NIH-TrialBench/patients.jsonl",
    split="train",
)

annotations = load_dataset(
    "json",
    data_files="hf://datasets/ncbi/NIH-TrialBench/clinician_annotations.jsonl",
    split="train",
)

trials = load_dataset(
    "json",
    data_files="hf://datasets/ncbi/NIH-TrialBench/trial_search_space.jsonl",
    split="train",
)
```
